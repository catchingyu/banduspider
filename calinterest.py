...
sellcost = 0.015
buycost = 0.0012
global_fundcode = '164809'
global_sdate = '2009-03-05'
global_edate = '2018-05-01'
...
def cal_possibility(fundcode,periodtime):
	...
	numofmax7 = 0
	numofallcaldays = 0
	edate = trans_sdate2datetime(global_edate)
	enddate = edate - datetime.timedelta(days=periodtime)
	for item in alldata:
		#print item[1]
		numofallcaldays += 1
		curdate = trans_sdate2datetime(item[0])
		if curdate > enddate :
			#print curdate
			break
		if item[1] > 1 :
	...
	print float(numofprofit)/numofallcaldays
	print float(numofmax3)/numofallcaldays
	print float(numofmax7) / numofallcaldays
	#print profitrate
...
def main(argv):
	getallfund.getallfund(global_fundcode,global_sdate,global_edate)
