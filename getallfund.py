	...
	for line in tr_re.findall(json_fund_value):
		...
		if match:
			...
			if bonusDay != "":
				...
				if bonusDay.find(u'现金') != -1:
					bonusDay = bonusDay[bonusDay.find(u'现金') + 2:bonusDay.rfind(u'元')]
				elif bonusDay.find(u'折算') != -1:
					bonusDay = bonusDay[bonusDay.find(u'折算') + 2:bonusDay.rfind(u'份')]
					#set to 0
					bonusDay = 0.0
				print bonusDay
...
def get_funInfo(db_fundcode, global_fundcode, global_sdate,global_edate):
	#hardcode test number
	strfundcode = global_fundcode
	strsdate = global_sdate
	stredate = global_edate
...
def getallfund(global_fundcode, global_sdate,global_edate) :
	db_fundcode = init_DB(global_fundcode)

	get_funInfo(db_fundcode,global_fundcode,global_sdate,global_edate)
